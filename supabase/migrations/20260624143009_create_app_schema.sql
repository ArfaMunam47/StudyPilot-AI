...
## Overview
Multi-user learning assistant app with auth, user profiles, assignments, study plans,
flashcards, quizzes, notes, and career data. All tables have RLS with user-scoped policies.

## New Tables
- `user_profiles` — extends auth.users with app-specific data
- `assignments` — user assignments with subjects and deadlines
- `study_plans` — daily/weekly study schedule items
- `flashcards` — user-created flashcards for spaced repetition
- `quizzes` — quiz results and generated quizzes
- `notes` — user notes with AI simplification flag
- `career_interests` — user's saved career paths
- `app_settings` — user preferences (dark mode, notifications)

## Security
All tables have RLS enabled. Each user can only access their own data via auth.uid().
*/
...
CREATE POLICY "select_own_quizzes" ON quiz_results FOR SELECT
  TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "insert_own_quizzes" ON quiz_results;
CREATE POLICY "insert_own_quizzes" ON quiz_results FOR INSERT
  TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "update_own_quizzes" ON quiz_results;
CREATE POLICY "update_own_quizzes" ON quiz_results FOR UPDATE
  TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "delete_own_quizzes" ON quiz_results;
CREATE POLICY "delete_own_quizzes" ON quiz_results FOR DELETE
  TO authenticated USING (auth.uid() = user_id);
